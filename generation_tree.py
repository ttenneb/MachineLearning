...
def alternating(matrix, func, out, bins, sample_size, weights, noise=.1):
    l = 0
    r = matrix.shape[0] -1
    org = out
    while l <= r:
        for element in matrix[l, l:r+1, :]:
            ...
            y_hat = func(sample_pixels_with_noise(org, x, y, sample_size, bins, noise).reshape(1, -1), weights)
...
def main():
    ...
    X, Y = generate_dataset(im, sample_size, 10, bins, p=0.3)
    ...
    clf.train(X.reshape(-1, sample_size*3), Y, max_depth=4, min_sample_size=10)
    ...
    im_alt = alternating(grid, clf.predict, im, bins, sample_size, bin_sizes, 0.3)
    plt.imshow(im_alt)
    plt.show()
